...
aws iam add-user-to-group --group-name $IAM_GROUP --user-name $DEPLOY_USER || exit 1
KEYS=`aws iam create-access-key --user-name $DEPLOY_USER`
DEPLOY_USER_ACCESS_KEY=`echo $KEYS | python -c 'import json,sys;obj=json.load(sys.stdin);print obj["AccessKey"]["AccessKeyId"]'`
DEPLOY_USER_SECRET_KEY=`echo $KEYS | python -c 'import json,sys;obj=json.load(sys.stdin);print obj["AccessKey"]["SecretAccessKey"]'`


puts "Applying security policy to IAM user '$DEPLOY_USER'..."
aws iam put-user-policy --user-name $DEPLOY_USER --policy-document file://$DEPLOY_USER_POLICY_FILE \
...
aws iam create-user --user-name $IN_INSTANCE_USER
aws iam add-user-to-group --group-name $IAM_GROUP --user-name $IN_INSTANCE_USER || exit 1
KEYS=`aws iam create-access-key --user-name $IN_INSTANCE_USER`
IN_INSTANCE_USER_ACCESS_KEY=`echo $KEYS | python -c 'import json,sys;obj=json.load(sys.stdin);print obj["AccessKey"]["AccessKeyId"]'`
IN_INSTANCE_USER_SECRET_KEY=`echo $KEYS | python -c 'import json,sys;obj=json.load(sys.stdin);print obj["AccessKey"]["SecretAccessKey"]'`

puts "Applying security policy to IAM user '$IN_INSTANCE_USER'..."
aws iam put-user-policy --user-name $IN_INSTANCE_USER --policy-document file://$IN_INSTANCE_USER_POLICY_FILE \
